...
namespace Xitils {
	...
	public:
		std::function<float(float)> f; // R -> [0, 1] ���`�P�������ł���K�v������
		...
		float normalizeValue(float x) const {
			...
			case Parameter1D::Loop::Reverse:
				if (y < 0) { y += 2 * (-floor(y)); } // �K���ȋ���𑫂��Đ��ɂ���
				y = clamp01(((int)y) % 2 == 0 ?
					y - floorf(y) // ������������̏ꍇ Repeat �Ɠ���
					: 1.0f - (y - floorf(y)) // ����������̏ꍇ ���]����
				);
				break;
			}
	...
	public:
		...
		//}
		//template <typename... Args> std::optional<T> eval( Args... args ) { // ��Ԃ���
		//	std::vector<TableBinIndex>index;
		//	return access( std::move( index ), args... );
		//}
		template <typename... Args> std::optional<ValueType*> access(Args... args) { // ��ԂȂ� �e�[�u���̒l�ɏ������݉\
