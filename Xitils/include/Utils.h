...
namespace Xitils {
	...
	template <typename ValueType> ValueType min(ValueType x, ValueType y) { return x < y ? x : y; }
	template <typename ValueType> ValueType max(ValueType x, ValueType y) { return x > y ? x : y; }
	template <typename ValueType> ValueType min(ValueType x, ValueType y, ValueType z) { return min(x, min(y, z)); }
	template <typename ValueType> ValueType max(ValueType x, ValueType y, ValueType z) { return max(x, max(y, z)); }

	template <typename ValueType> ValueType clamp(ValueType x, ValueType minVal, ValueType maxVal) { return max(minVal, min(maxVal, x)); }
	template <typename ValueType> ValueType clamp01(ValueType x) { return clamp(x, (ValueType)0.0f, (ValueType)1.0f); }
	template <typename ValueType> ValueType clampPositive(ValueType x) { return max(x, (ValueType)0.0f); }
